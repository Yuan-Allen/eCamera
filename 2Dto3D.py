...
window_name = "Frame"
camera_intrinsic = np.mat(np.loadtxt("./camera_matrix.txt"))
r = np.loadtxt("./rotM.txt")
t = np.asmatrix(np.loadtxt("./translation_vector.txt")).T
...
def on_EVENT_LBUTTONDOWN(event, x, y, flags, param):
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        print("({}, {}) --> ({}, {}, {}))".format(x, y, result[0][0][0], result[0][0][1], result[0][0][2]))
